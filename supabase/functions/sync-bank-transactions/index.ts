...
async function syncOneConnection(
  ...
  while (hasMore) {
    const res = await fetch(
      `https://${PLAID_ENV}.plaid.com/transactions/sync`,
      {
        method: "POST",
    ...
    if (!res.ok) {
      console.error(
        "Plaid /transactions/sync failed for connection",
        connection.id,
        res.status,
        await res.text(),
      );
      ...
      return {
        connectionId: connection.id,
        synced: syncedCount,
        error: "Plaid sync request failed",
      };
    }
...
serve(async (req: Request) => {
  ...
  if (cronSecret) {
    ...
    for (const connection of connections)
      results.push(await syncOneConnection(admin, connection));
  ...
  try {
    ...
    console.error(
      "Unexpected error in sync-bank-transactions:",
