...
const CATEGORY_TYPES: Record<string, 'income' | 'expense'> = {
  ...
  food: 'expense', rent: 'expense', transport: 'expense', entertainment: 'expense',
  shopping: 'expense', utilities: 'expense', health: 'expense',
  savings: 'expense', debt: 'expense', other: 'expense',
};
...
const KEYWORD_CATEGORIES: Array<[RegExp, string]> = [
  ...
  [/\b(lunch|dinner|breakfast|coffee|cafe|restaurant|food|groceries|grocery|snack|eat|meal)\b/i, 'food'],
...
function heuristicParse(text: string, today: string) {
  ...
  for (const [pattern, id] of KEYWORD_CATEGORIES) {
    if (pattern.test(text)) { category = id; break; }
  }
  ...
  return { description: description.charAt(0).toUpperCase() + description.slice(1), amount, category, date, confidence: 0.5 };
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (text.length > MAX_INPUT_CHARS) return json({ error: `Keep it under ${MAX_INPUT_CHARS} characters.` }, 400);
    ...
    if (!raw) return json({ error: "Couldn't find an amount in that. Try \"Spent 15 on lunch today\"." }, 422);
    ...
    const signedAmount = CATEGORY_TYPES[category] === 'income' ? amount : -amount;
    ...
    const { data: inserted, error: insertError } = await userClient
      .from('transactions')
      .insert({ user_id: user.id, description, amount: signedAmount, category, date })
      .select('id, description, amount, category, date, created_at')
      .single();

    if (insertError) {
      console.error('insert failed', insertError.message);
      return json({ error: "Couldn't save that transaction. Please try again." }, 500);
    }
