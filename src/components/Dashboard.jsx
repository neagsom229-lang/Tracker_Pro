...
export default function Dashboard() {
  ...
  // underlying totals.
  const { totalBalance, totalIncome, totalExpenses } = useMemo(() => selectTotals(transactions), [transactions]);
  const spendingThisMonth = useMemo(() => selectSpendingThisMonth(transactions), [transactions]);

  return (
    <div className="flex flex-col gap-6">
      {/* Top of the dashboard on purpose: this is the fastest path from
          "I just spent money" to "it's recorded", and it's the feature
          that differentiates the product — burying it below the fold
          would be hiding the thing people came back for. */}
      <AIQuickAdd />
